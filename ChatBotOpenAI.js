class ChatWindow {
    init(config) {
        ...
        this.getRoomId();
        this.loadCSS('https://coquan.vn/sample.css');
        console.log(this);
        // this.getParams();
 ...
 */
    ...
    alert(message, options) {
        let that = this;
        that.loadCSS(`/3rdparty/jQuery/lobibox/css/lobibox.min.css`);
        that.loadScript(`/3rdparty/jQuery/lobibox/js/lobibox.min.js`)
            .then(function () {
                if (!options) {
                    options = {
                        closeOnClick: true,
    ...
    addMessage(res, input) {
        ...
        let updateTime = (timeEl, createdTime) => {
            let currentTime = new Date(),
                timeDifference = currentTime - createdTime + ((59 + 15) * 1000),
            ...
            timeEl.textContent = `${timeDifferenceInMinutes >= 60 ? Math.floor(timeDifferenceInMinutes / 60) : timeDifferenceInMinutes}  ${timeLabel} trước`;
        };
