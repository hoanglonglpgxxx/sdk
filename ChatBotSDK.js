class ChatWindow {
    init(config) {
        ...
        this.getRoomId();
        this.loadCSS('https://coquan.vn/sdk.css');
        console.log(this);
        // this.getParams();
 ...
 */
    ...
    getRoomId() {
        let that = this;
        let url = `https://hcc04.coquan.vn/api/Project/STDV/ChatBot/Chat/select`,
            roomId;
        fetch(url, {
            method: 'GET',
        })
    ...
    alert(message, options) {
        let that = this;
        that.loadCSS(`https://coquan.vn//3rdparty/jQuery/lobibox/css/lobibox.min.css`);
        that.loadScript(`https://coquan.vn//3rdparty/jQuery/lobibox/js/lobibox.min.js`)
            .then(function () {
                if (!options) {
                    options = {
                        closeOnClick: true,
 ...
 */
    ...
    submitFunc(el, roomId = '', isOption = false) {
        let that = this;
        let url = `https://hcc04.coquan.vn/api/Project/STDV/ChatBot/Chat/send?text=${isOption ? el : el.innerHTML}&roomId=${roomId}&site=${that.siteId}`;
        console.log(roomId);
        fetch(url, {
            method: 'POST',
    ...
    addMessage(res, input) {
        ...
        let updateTime = (timeEl, createdTime) => {
            let currentTime = new Date(),
                ...
                timeLabel = timeDifferenceInMinutes >= 60 ? 'giờ' : (timeDifference / 1000 >= 60 ? 'phút' : 'giây');
            timeDifferenceInMinutes = timeDifferenceInMinutes >= 60 ? Math.floor(timeDifferenceInMinutes / 60) : timeDifferenceInMinutes;
            timeEl.textContent = `${timeDifferenceInMinutes}  ${timeLabel} trước`;
        };
